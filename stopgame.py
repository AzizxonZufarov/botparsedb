...
class StopGame:
	...
	def new_games(self):
		...
		new = []
		items = html.select('._card_8sstg_1 > a')
		for i in items:
	...
	def game_info(self, uri):
		...
		poster = re.match(r'background-image:\s*url\((.+?)\)', html.select('._card__image_8sstg_399')[0]['style'])

		# remove some stuff
		remels = html.select('._card__bottom_8sstg_1')
		for remel in remels:
			remel.extract()
		...
		info = {
			...
			"title": html.select('._card__title_8sstg_1 > a')[0].text,
			"link": link,
